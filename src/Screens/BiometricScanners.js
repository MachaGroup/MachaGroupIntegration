...
const biometricScannerQuestions = [
    { name: "biometricScannersOperational", label: "Are the biometric scanners operational and functioning as intended?" }, // Typo fixed: Opertaional -> Operational
    ...
    { name: "biometricEnrollment", label: "Are individuals required to enroll their biometric data for access?" }, // Simplified label
    { name: "restrictedAccess", label: "Is access restricted to individuals whose biometric data matches stored records?" },
    { name: "updateRecordsProcess", label: "Is there a process to update/remove biometric records as needed (e.g., personnel changes)?" }, // Simplified label
    ...
    { name: "monitoring", label: "Is there real-time monitoring and logging of access events from scanners?" }, // Simplified label
    { name: "centralManagement", label: "Are access rights managed centrally and synchronized with the biometric system?" },
    { name: "securityFeatures", label: "Are the scanners equipped with security features to prevent tampering or spoofing?" }, // Simplified label
    { name: "antiSpoofing", label: "Do they employ anti-spoofing measures to detect fraudulent authentication attempts?" }, // Simplified label
    { name: "physicalSecurity", label: "Are physical security measures in place to protect scanner components?" }, // Simplified label
    ...
    { name: "testingCertification", label: "Have the scanners undergone testing or certification for compliance?" }, // Simplified label
    ...
    { name: "userTraining", label: "Have users (security, staff, individuals) received training on proper scanner usage?" }, // Simplified label
    { name: "instructions", label: "Are instructions available regarding enrollment and access procedures?" }, // Simplified label
...
function BiometricScannersPage() {
  ...
  const uploadBiometricScannersImage = httpsCallable(functions, 'uploadBiometricScannersImage');
  ...
  const handleSubmit = async (e) => {
    ...
    if (imageData) {
      try {
        ...
        const uploadResult = await uploadBiometricScannersImage({
            imageData: imageData,
      ...
      <main className="form-container">
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-section">
              <label htmlFor="imageUploadBiometric">Upload Image (Optional):</label>
